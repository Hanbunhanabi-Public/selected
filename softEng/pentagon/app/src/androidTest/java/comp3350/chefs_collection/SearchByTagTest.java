...
public class SearchByTagTest {
    ...
    @Test
    public void testViewTagList() {
        RecipeUtils.navigateToRecipe(0);
        onData(anything()).inAdapterView(withId(R.id.tagGrid)).atPosition(0).perform(click());
        onData(anything()).inAdapterView(withId(R.id.recipeListView)).atPosition(0).onChildView(withId(R.id.recipe_name)).check(matches(withText(RecipeUtils.NAME)));

    }
    @Test
    public void testViewTagListLink() {
        RecipeUtils.navigateToRecipe(0);
        onData(anything()).inAdapterView(withId(R.id.tagGrid)).atPosition(0).perform(click());
