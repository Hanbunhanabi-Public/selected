  ...
  // If you want to use this JSON library, https://github.com/nlohmann/json for more information. The code below gives examples of everything you should need: getting named values, iterating over arrays, and converting types.
	...
	for (json::iterator it = objects.begin(); it != objects.end(); ++it) {
		...
		if (object["type"] == "sphere") {
			...
			IntersectionResult result = sphere->intersectionTest(vector_to_vec3(pos), object["radius"], e, s);
			if (result.isIntersect && pick) {
				float distance = glm::distance(result.intersectionNormal, sphere->center);
				...
				std::cout << "normalDistance" << distance << std::endl;
			}
