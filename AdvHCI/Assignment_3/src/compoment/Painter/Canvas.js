...
class Canvas {
  ...
  shapeList = {};
  backBackground = {r: 0.2, g: 0.2, b: 0.2, a: 1};
  world = null;
  shapeIDAssign = 2;
  programInfo = null;
  gl = null;
  ...
  onContextCreate(gl, width, height, left = 0, top = 0) {
    console.log('gl size ', gl.drawingBufferWidth, gl.drawingBufferHeight);
  ...
  drawShape(gl, renderInstance, programInfo, isFill, color) {
    ...
    // console.log('===> draw shape:', renderInstance.shapeType, isFill);
  ...
  drawRing(ring, isFill = false) {
    ...
    this.setupProgram(this.gl, programInfo.program);
    //render each shape
    ...
    //     : this.renderList[i].counter_color
    //   : this.renderList[i].idColor;

    this.drawShape(this.gl, ring, programInfo, isFill, color);
    // }
  ...
  //render the current path separately
  //when draw on the back buffer the current path should be clear
  internalDraw(gl, is_full, isSelect = false) {
  ...
  draw(is_full, update_back_buffer) {
    const gl = this.gl;
    //normal render
  ...
  singleSelectTest(x, y) {
    ...
    x = x * widthRatio + this.gl.drawingBufferWidth / 2;
    y =
      this.gl.drawingBufferHeight -
      (this.gl.drawingBufferHeight / 2 - y * heightRatio);
    this.gl.readPixels(
      x, // x
      y, // y
      1, // width
